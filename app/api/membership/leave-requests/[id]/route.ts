...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (action === "approve") {
    ...
    const tx = (db as any).transaction(() => {
      ...
      db.prepare(`
        INSERT OR IGNORE INTO entity_members (id, entityId, userId, leftAt)
        VALUES (?, ?, ?, datetime('now'))
    ...
    try {
      tx();
      const updated = db.prepare(`SELECT status FROM leave_requests WHERE id=?`).get(id) as any;
