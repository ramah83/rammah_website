...
export async function GET(req: NextRequest) {
  try {
    ...
    if (scope === "mine" && viewerId) {
      if (roleParam === "entityManager" || s?.role === "entityManager") {
        const row =
          db
            .prepare(
              `
              SELECT e.id, e.name, e.type, e.contactEmail, e.phone, e.location, e.documents,
                     e.createdAt, e.createdBy, e.managerUserId, e.status
                FROM entities e
               WHERE e.managerUserId = ?
                  OR EXISTS(SELECT 1 FROM users u WHERE u.id = ? AND u.entityId = e.id)
               LIMIT 1
            `
            )
      ...
      const rows = db
        .prepare(
          `SELECT id, name, type, contactEmail, phone, location, documents,
                  createdAt, createdBy, managerUserId, status
             FROM entities
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "ممنوع إنشاء كيان لهذا الدور" },
      { status: 403 }
    );
  } catch (err: any) {
    return NextResponse.json(
