...
const ALLOWED_ENTITY_NAMES: string[] = [
  "كيان سند شباب الصعيد",
  "كيان اتحاد شباب الاقصر",
  "كيان رواد التطوير و التنميه الشبابيه بمحافظه",
  "كيان المصريون الشباب",
  "كيان اراده شباب مصر",
  "كيان شباب بحري",
  "رواد المحافظات الحدوديه",
  "كيان الجبهه الدبلوماسيه المصريه",
  "كيان كوادر شباب مصر",
  "كيان اتحاد طلاب مصر",
  "كيان سند شباب الدلتا",
  "كيان الجيل الشبابي الصاعد",
  "كيان الشباب بناة المستقبل",
  "كيان قيادات شباب مصر",
  "كيان مشروع وطن",
  "كيان اتحاد شباب كفر الشيخ",
  "كيان الاتحاد الشبابي لدعم مصر",
  "كيان إرادة شباب مصر بالغربية",
  "كيان شباب مستدام",
  "كيان رحلة شباب الجمهورية الجديدة",
  "كيان أكاديمية الكوري بسوهاج",
  "كيان أتحاد شباب الوطن بسوهاج",
  "كيان حلم مصر بسوهاج",
  "كيان تيم القمه بسوهاج",
  "كيان تيم الشيمي بسوهاج",
  "كيان جيل قادر بسوهاج",
  "كيان مراكز شباب مصر",
  "كيان فكرة",
  "الاتحاد الوطني للقيادات الشبابية",
  "كيان فن إدارة الحياة",
  "كيان تنمية وطن",
  "كيان حكاية إشارة",
  "كيان رموز شباب مصر",
  "كيان شباب يبني وطن",
...
export default function EntitiesPage() {
  ...
  // اختيار تلقائي: كيان المستخدم إن وجد وإلا أول كيان
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    let base = list;
    if (session?.role === "entityManager") base = base.filter(e => e.createdBy === session.id || e.id === session.entityId);
    if (!q) return base;
        ...
        <div className="rounded-[22px] p-5 md:p-6 flex items-center justify-between" style={{ backgroundColor: "#FFFFFF", border: "1px solid #E7E2DC", boxShadow: "0 8px 18px rgba(0,0,0,0.05)", color: "#1D1D1D" }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: "#6B6B6B" }}>اختر كيانًا موجودًا وحدّث بياناته (لا يتم إنشاء كيانات جديدة)</p>
            </div>
          </div>
      ...
      <main className="relative z-10 mx-auto max-w-6xl w-full px-4 mt-6 space-y-6 pb-10" style={{ color: "#1D1D1D" }}>
        ...
        <SurfaceCard className="mx-3 sm:mx-[1cm]">
          <CardHeader className="pb-0 px-5 pt-5">
            ...
            <CardDescription style={{ color: "#6B6B6B" }}>ابحث أو حرّر أي كيان</CardDescription>
          </CardHeader>
          ...
          <CardContent className="px-5 pb-5">
            <div className="mb-4 flex items-center gap-2">
              <div className="relative w-full md:w-80">
                ...
                <Input placeholder="ابحث بالاسم/المدينة/البريد..." className="pr-9 h-11 rounded-xl" style={{ backgroundColor: "#FFFFFF", color: "#1D1D1D", borderColor: "#E3E3E3" }} value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
...
function HeaderBar() {
  ...
  useEffect(() => { try { const s = localStorage.getItem("session"); if (s) setRole(JSON.parse(s).role); } catch {} }, []);
