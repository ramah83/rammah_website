...
export default function ISOPage() {
  ...
  useEffect(() => {
    ...
    setForm(p => ({ ...p, ownerEntityId: String(def) }));
  }, [session, entities]); 

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    return (list || [])
      .filter((f: any) => (filterEntity === "all" ? true : String(f.ownerEntityId) === String(filterEntity)))
  ...
  const resetForm = () =>
    setForm({ code: "", title: "", ownerEntityId: String(session?.entityId || entities[0]?.id || ""), status: "draft" });
  ...
  const onSave = async (e: React.FormEvent) => {
    ...
    try {
      await api.createISO({
        ...
        status: form.status,
        ownerEntityId: String(form.ownerEntityId),
      });
  ...
  const confirmEdit = async () => {
    ...
    try {
      await api.updateISO(editingId, {
        ...
        ownerEntityId: String(editDraft.ownerEntityId),
                        ...
                        <div className="w-full space-y-2">
                          {isEdit ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              ...
                              />
                              <div className="md:col-span-1">
                                <Select value={editDraft?.ownerEntityId || f.ownerEntityId}
                              ...
                              <div className="md:col-span-1">
                                <Select value={editDraft?.status || f.status}
  ...
  return (
    <header className="relative z-10">
      <div className="mx-auto max-w-6xl px-4">
        <div className="mt-4 h-14 w-full rounded-٢xl flex items-center justify-between px-4"
